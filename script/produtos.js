...
function mostrarMensagem(texto, tipo) {
    // Redireciona diretamente para o sistema Toast moderno
    mostrarNotificacao(texto, tipo, 2500);
}

// Envia as propriedades estruturadas para o banco persistente do navegador
function addCarrinho(id, name, price, img, description) {
    ...
    if (produtoExistente) {
        // Se já existir na lista, incrementa a quantidade
        produtoExistente.qty += 1;
    } else {
        // Se for um item inédito, monta o objeto completo esperado pela tela do carrinho
        var novoProduto = {
            id: id,
            name: name,
            price: price,
    ...
    // Aciona a mensagem de feedback azul elétrico
